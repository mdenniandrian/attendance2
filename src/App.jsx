...
export default function ShiftApp() {
  ...
  // App View & Auth States
  const [activeView, setActiveView] = useState('dashboard'); // 'dashboard' | 'backoffice'
  const [backofficeTab, setBackofficeTab] = useState('employees'); // 'employees' | 'schedule' | 'data'
  ...
  // Handle switching to Backoffice with authentication guard
  ...
  // Handlers for persistent state updates
  const handleSaveEmployees = (newEmployees) => {
  ...
  const handleDatabaseReload = (newEmployees, newOverrides) => {
    setEmployees(newEmployees);
    setOverrides(newOverrides);
  };
  ...
  // Hitung rekapitulasi shift (Tally)
  const activeTallyEmployees = isYearlyPrint
    ? sortEmployees(employees.filter(e => e.status !== 'inactive'))
    : getActiveEmployeesForMonth(employees, year, month);

  const shiftTallies = activeTallyEmployees.map(emp => {
    const tally = {
      'P (L1)': 0,
      'P2 (L1)': 0,
      'S (L1)': 0,
    ...
    renderMonths.forEach(m => {
      const activeInM = getActiveEmployeesForMonth(employees, year, m).some(e => e.id === emp.id);
      if (isYearlyPrint && !activeInM) return;

      const daysInM = new Date(year, m + 1, 0).getDate();
      for (let d = 1; d <= daysInM; d++) {
        const shift = resolveShift(emp, year, m, d, overrides);
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-4 md:p-10 font-sans print:bg-white print:p-0 print:min-h-0 text-slate-800 relative selection:bg-indigo-100 selection:text-indigo-900">
      ...
      {/* Watermark Maxcloud (Hanya tampil saat print) */}
      ...
      {activeView === 'backoffice' && isAuthenticated && (
        <div className="max-w-7xl mx-auto space-y-6 print:hidden animate-in fade-in duration-300">
          ...
          <div className="bg-white/85 backdrop-blur-2xl p-6 rounded-3xl border border-white/80 shadow-xl shadow-slate-200/50 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="px-4 py-2.5 rounded-2xl bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold text-xs border border-rose-200 transition-all flex items-center gap-1.5 shadow-sm"
          ...
          <div className="bg-white/70 backdrop-blur-xl p-2 rounded-2xl border border-white flex flex-wrap gap-2 shadow-sm">
            <button
              type="button"
              onClick={() => setBackofficeTab('employees')}
              className={`px-5 py-3 rounded-xl text-xs font-black transition-all flex items-center gap-2 ${
                backofficeTab === 'employees'
                  ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200'
                  : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-100'
              }`}
            >
              <span>👥</span>
              <span>Manajemen Karyawan ({employees.length})</span>
            </button>

            <button
              type="button"
              onClick={() => setBackofficeTab('schedule')}
              className={`px-5 py-3 rounded-xl text-xs font-black transition-all flex items-center gap-2 ${
                backofficeTab === 'schedule'
                  ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200'
                  : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-100'
              }`}
            >
              <span>📅</span>
              <span>Editor Jadwal & Shift Override</span>
            </button>

            <button
              type="button"
              onClick={() => setBackofficeTab('data')}
              className={`px-5 py-3 rounded-xl text-xs font-black transition-all flex items-center gap-2 ${
                backofficeTab === 'data'
                  ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200'
                  : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-100'
            ...
            >
              <span>💾</span>
              <span>Data, Keamanan & Cadangan</span>
            </button>
          </div>
            ...
            <tr className="block print:table-row">
              <td className="block print:table-cell">
                ...
                {renderMonths.map((m) => {
                  ...
                  const activeEmployees = getActiveEmployeesForMonth(employees, year, m);
      ...
      {/* Informasi & Aturan Shift (Screen Only) */}
      <div className={`max-w-7xl mx-auto mt-12 bg-white/60 backdrop-blur-xl rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-white p-8 md:p-12 print:hidden relative overflow-hidden group ${
        activeView === 'backoffice' ? 'hidden' : 'block'
      }`}>
