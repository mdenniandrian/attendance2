...
export default function ShiftApp() {
  ...
  const holidaysToShow = isYearlyPrint ? holidays : holidays.filter(h => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-sky-50 p-4 md:p-8 font-sans print:bg-white print:p-0 print:min-h-0 text-slate-800">
      ...
      <div id="pdf-container" className="relative max-w-6xl mx-auto bg-white/90 backdrop-blur-xl rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] border border-white print:bg-white print-card print:border-none print:shadow-none print:rounded-none print:overflow-visible">
        ...
        {/* Header & Controls */}
        <div className="relative bg-gradient-to-r from-blue-700 via-indigo-600 to-indigo-800 text-white p-6 sm:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 print:bg-transparent print:bg-none print:text-black print:p-0 print:mb-4">
          ...
          <div className="relative z-20 print:hidden">
            <button
              ...
              className="bg-white text-indigo-700 hover:bg-indigo-50 font-extrabold py-3 px-6 rounded-full flex items-center gap-2 transition-all duration-300 hover:scale-105 shadow-[0_10px_25px_-5px_rgba(255,255,255,0.4)] shadow-lg whitespace-nowrap border border-white"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M6 2a2 2 0 00-2 2v12a2 2 0 002 2h8a2 2 0 002-2V7.414A2 2 0 0015.414 6L12 2.586A2 2 0 0010.586 2H6zm5 6a1 1 0 10-2 0v3.586l-1.293-1.293a1 1 0 10-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L11 11.586V8z" clipRule="evenodd" />
              </svg>
              Export PDF
              <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ml-1 transition-transform ${isExportMenuOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            
            {isExportMenuOpen && (
              <div className="absolute right-0 mt-2 w-40 bg-white rounded-xl shadow-xl border border-indigo-100 overflow-hidden flex flex-col z-50">
                <button
                  onClick={() => { setIsExportMenuOpen(false); handleExportPDF(); }}
                  className="px-4 py-3 text-left text-sm font-bold text-indigo-900 hover:bg-indigo-50 border-b border-indigo-50 transition-colors"
                >
                  Bulanan
                </button>
                <button
                  onClick={() => { setIsExportMenuOpen(false); handleExportYearlyPDF(); }}
                  className="px-4 py-3 text-left text-sm font-bold text-indigo-900 hover:bg-indigo-50 transition-colors"
                >
                  1 Tahun
                </button>
              </div>
        ...
        {renderMonths.map((m) => {
          ...
          return (
            <div key={m} className="print:break-inside-avoid print:mb-6">
              <div className="px-2 sm:px-4 md:px-6 pb-4 sm:pb-6 overflow-x-hidden print:overflow-visible print:p-0">
                <div className="hidden print:block text-xs font-bold text-gray-700 mb-1.5 text-right w-full uppercase">
                ...
                <table className="w-full table-fixed text-sm text-center border-collapse">
                  <thead>
                    <tr>
                      ...
                      {dates.map(date => {
                        const dayIdx = getDayIndex(year, m, date);
        ...
        <div className="hidden print:flex justify-between items-start pt-4 border-t border-gray-200 mx-6 mb-2 mt-2 print:break-inside-avoid">
          {/* Tabel Rekapitulasi */}
          <div className="w-auto">
            <h3 className="font-extrabold text-black text-[10px] uppercase tracking-wider mb-2">
              Rekapitulasi Shift {isYearlyPrint ? 'Satu Tahun' : 'Bulan Ini'}
            </h3>
            <table className="w-auto text-[9px] text-center border-collapse">
              <thead>
                <tr className="bg-gray-100">
                  <th className="p-1.5 border border-gray-300 text-left">Karyawan</th>
            ...
            <div className="mt-4">
              ...
              {holidaysToShow.length > 0 ? (
                <ul className="text-[9px] list-disc list-inside text-gray-800 columns-1 sm:columns-2 print:columns-2">
                  {holidaysToShow.map((h, i) => {
                ...
                <p className="text-[9px] text-gray-800 italic">tidak ada tanggal merah/libur nasional</p>
              )}
            </div>
          </div>
