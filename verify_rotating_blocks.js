...
function getShift(empName, dayIndex) {
  ...
  // Hari libur masing-masing karyawan untuk minggu ini
  ...
  if (D < 3) { // Senin - Rabu: Full Team
    ...
    // Cari tahu siapa karyawan yang libur hari ini
    const p_off = mod(D + 3 - W, 4);
    
    // Tentukan base shift dari karyawan yang libur hari ini
    let offBaseShift;
    if (p_off === 0) {
      offBaseShift = baseShifts[mod(1 + W, 4)];
    } else if (p_off === 1) {
      offBaseShift = baseShifts[mod(0 + W, 4)];
    } else if (p_off === 2) {
      offBaseShift = baseShifts[mod(2 + W, 4)];
    } else {
      offBaseShift = baseShifts[mod(3 + W, 4)];
    }

    // Karyawan yang mendapatkan base shift P2 pada hari ini bertugas meng-cover shift karyawan yang libur
